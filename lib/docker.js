...
DeconstDocker.prototype.runContainer = function (options, callback) {
  ...
  if (options.workspace) {
    ...
    if (self.toolbelt.workspaceContainer()) {
      ...
      // options.workspace.root is a path within the workspace volume.
      // options.workspace.rootEnvVar is an environment variable that should be set to the
      //  workspace location.
      options.HostConfig.VolumesFrom = [self.toolbelt.workspaceContainer()]
      options.Env.push(options.workspace.rootEnvVar + '=' + options.workspace.root)
      options.Env.push('NPM_CONFIG_CACHE=' + path.join(options.workspace.root, '.npmcache'))
      options.Env.push('TMPDIR=' + path.join(options.workspace.root, '.tmp'))
    } else {
      // Mount the workspace volume from the local filesystem, instead.
      // options.workspace.root is a path on the local filesystem.
      // options.workspace.containerRoot is a path within the container filesystem to which the
      //   local directory should be mounted.
      if (!options.Mounts) options.Mounts = []
      if (!options.HostConfig.Binds) options.HostConfig.Binds = []

      var volumeRoot = options.workspace.root
      var containerPath = options.workspace.containerRoot
      ...
      options.HostConfig.Binds.push(bind)
    }
